 ...
 * spread bps, fractional-share toggle, default borrow rate). Used by the
 * /paper buy form to surface the fractional mode, and by /settings to render the
...
export const RiskSchema = z.object({
  ...
  spread_bps: z.number().min(0).max(200).optional(),
...
export async function PATCH(req: Request) {
  try {
    ...
    if (parsed.data.spread_bps != null) pairs.push(["risk.spread_bps", String(parsed.data.spread_bps)]);
